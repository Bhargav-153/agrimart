...
const AddCropProtection = () => {
  ...
  const onSubmit = async (values) => {
    if (!fileBase64) return showToast("error", "Product image is required");
    try {
  ...
  const { data: cropData } = useFetch(`${API_BASE_URL}/crop-protection/all`, {}, [refreshData]);
  

  const handleDelete = async (id) => {
    try {
      const res = await fetch(`${API_BASE_URL}/crop-protection/delete/${id}`, { method: "DELETE" });
                ...
                <TableRow key={i}>
                  ...
                  <TableCell className="flex gap-3">
                    <Button variant="outline" className="w-10 hover:bg-green-500 hover:text-white" asChild>
                      <Link to={RouteCropProtectionEdit(p._id)}><FaEdit /></Link>
                    </Button>
